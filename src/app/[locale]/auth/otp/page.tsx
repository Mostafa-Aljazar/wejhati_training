...
export default function OTP() {
  ...
  const [seconds, setSeconds] = useState(() => Math.max(60 - Math.floor((Date.now() - query.date) / 1000), 0))
  ...
  const form = useForm({
    ...
    validate: zodResolver(z.object({
      otp: z.string().length(4, errors("Invalid_otp")).regex(/^\d{4}$/, errors("Invalid_otp")),
    })),
  })
  ...
  useEffect(() => {
    if (seconds > 0) {
      const interval = setInterval(() => 
        setSeconds(prev => prev > 1 ? prev - 1 : 0), 1000)
  ...
  return (
    <div className=" h-full flex-col items-center justify-start bg-white lg:pt-16  md:justify-center md:rounded-[12px] lg:flex lg:w-[550px]">
      <p className="text-2xl lg:text-3xl text-center font-medium">{t("title")}</p>
      <p className="mt-3 w-[345px] text-center text-sm font-normal text-[#817C74]">
        {t("text")} <span>{query.email}</span>
      </p>

      <form className="mt-16 flex w-full flex-col items-center justify-center" onSubmit={handleSubmit}>
        <div className="flex flex-col items-center gap-3">
          ...
          />
          {!seconds && (
            <span className="text-sm font-normal text-[#FD6265]">{errors("Time_out")}</span>
          )}
        </div>
        ...
        {!seconds && (
          <Button
            ...
            disabled={isLoading}
          >
            <span className="text-base font-semibold text-[#F19A07] underline">{t("resend")}</span>
          </Button>
        )}
        ...
        <Button
          ...
          className={cn(
            ...
            !seconds || form.values.otp.length !== 4 ? "bg-[#F19A0763]" : "bg-[#F19A07]"
          )}
          w={228}
        >
          {t("button")}
